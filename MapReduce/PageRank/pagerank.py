...
if __name__ == "__main__":
    ...
    # Load input file
    lines = sc.textFile("gs://pagerank-bucket1/pagerank_data.txt")

    # Parse neighbors
    links = lines.map(parseNeighbors).filter(lambda x: x is not
None).distinct().groupByKey().cache()
    # Initialize ranks
    ...
    for iteration in range(iterations):
        ...
        contribs = links.join(ranks).flatMap(lambda url_urls_rank:
         ...
         # Update ranks
    ranks = contribs.reduceByKey(lambda x, y: x + y).mapValues(lambda rank: 0.15 + 0.85
* rank)
